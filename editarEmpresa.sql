CREATE DEFINER=`admin`@`localhost` PROCEDURE `editarEmpresa`(
...
BEGIN 
    ...
    START TRANSACTION;
        ...
        BEGIN
		IF EXISTS(SELECT * FROM domicilio WHERE id=_id) THEN
        ...
        ELSE
			ROLLBACK;
            SET _error = CONCAT(_error, 'No existe en la base de datos.');
        END IF;
    END;
